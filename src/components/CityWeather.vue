<template>
  <div class="flex flex-col flex-1 items-center">
    ...
    <div v-if="route.query.preview" class="text-white p-4 bg-weather-secondary w-full text-center">
      <p>
        You are currently previewing {{ route.params.city }}, click the "+" icon to start tracking
        the city.
      </p>
    </div>
    <!-- Weather Overview -->
  ...
  try {
    const weatherData = await axios.get(
      `https://api.openweathermap.org/data/2.5/onecall?lat=${route.query.lat}&lon=${
        route.query.lng
      }&exclude={part}&appid=${import.meta.env.VITE_OPEN_WEATHER_API_KEY}&units=metric`
    );
